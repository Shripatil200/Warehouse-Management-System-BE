...
public interface BinRepository extends JpaRepository<StorageBin, String> {
    ...
    /**
     * PUTAWAY ENGINE QUERY: Finds active bins that have enough space.
     * We filter by status (AVAILABLE) and check if current occupancy < capacity.
     */
    @Query("SELECT b FROM StorageBin b WHERE b.active = true " +
            "AND b.status = 'AVAILABLE' " +
            "AND (b.capacity - b.currentOccupancy) >= :requiredSpace " +
            "ORDER BY (b.capacity - b.currentOccupancy) DESC") // Best-fit: biggest space first
    List<StorageBin> findAvailableBinsForPutaway(@Param("requiredSpace") Integer requiredSpace);
}